...
categories = [item['category'] for item in faq_data]  # Extract category
hits = [item['hit'] for item in faq_data]  # Extract hit
...
def retrieve_and_generate():
    try:
        ...
        retrieved_category = categories[matched_index]  # Use the category field
        retrieved_hit = hits[matched_index]  # Use the hit field
...
"""
        ...
        model = genai.GenerativeModel("gemini-2.0-flash-lite-001")  # Updated to a valid model name
        response = model.generate_content(prompt)
        return jsonify({"response": response.text})
        #return jsonify({"response": response.text},{"category" : retrieved_category})
